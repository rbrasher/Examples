...
public class CardinalSplineMoveModifierExample extends SimpleBaseGameActivity {
	...
	private void addRectangleWithTension(final Scene pScene, final float pTension, float pDelay) {
		...
		rectangle.setBlendFunction(GLES20.GL_SRC_ALPHA, GLES20.GL_ONE_MINUS_SRC_ALPHA);
		if(pTension < 0) {
		...
		for(int i = 0; i < CardinalSplineMoveModifierExample.CONTROLPOINT_1_XS.length; i++) {
			...
			catmullRomMoveModifierConfig2.setControlPoint(i, CardinalSplineMoveModifierExample.CONTROLPOINT_2_XS[i] - SIZE / 2, CardinalSplineMoveModifierExample.CONTROLPOINT_YS[1] - SIZE / 2);
		}
