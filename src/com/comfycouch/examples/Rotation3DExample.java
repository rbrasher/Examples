...
public class Rotation3DExample extends SimpleBaseGameActivity {
	...
	protected Scene onCreateScene() {
		...
		/* Calculate center coordinates for the face, so it's centered on the camera */
		...
		final Sprite face = new Sprite(centerX, centerY, this.mFaceTextureRegion, this.getVertexBufferObjectManager()) {
			...
			protected void applyRotation(final GLState pGLState) {
				...
				if(rotation != 0) {
					...
					pGLState.translateModelViewGLMatrixf(rotationCenterY, rotationCenterY, 0);
					/* Note: we are applying rotation around the y-axis and not the x-axis anymore! */
